...
function makeObject(path, type, jsonData, IUSSWID, xenomeData, callback) {
	var obj = {};
	if (type=="dna") {
		obj['type'] = 'dna';
					...
					getInsertDistribution(lineObj, IUSSWID, path, function (insertFile) {
						...
						getSoftClip (lineObj, IUSSWID, path, function (softClipFile) {
							...
							obj['soft_clip_by_cycle'] = softClipFile;
								return callback(obj, path, IUSSWID);
						});
					});
				});
			} else {
...
function getReadBreakdown(lineObj, IUSSWID, path, callback){ 
	...
	if (graphExists) {
		...
		stream.once('open', function(fd) {
			...
			var line8 = "pie(slices, labels = lbls, col=cols, main=\"Read Breakdown\", border=NA)\n";
			var line9 = "dev.off()\n";
...
function getInsertDistribution(lineObj, IUSSWID, path, callback){
	...
	if (graphExists) {
		...
		stream.once('open', function(fd) {
			...
			var line5 = "plot(xvals, yvals, main=\"Insert Distribution\", type=\"n\", col=\"black\", xlab=\"Insert Size (bp)\", ylab=\"Pairs\")\n";
...
function getSoftClip(lineObj, IUSSWID, path, callback){
	...
	if (graphExists) {
		...
		stream.once('open', function(fd) {
			...
			var line5 = "plot(xvals, yvals, main=\"Soft Clips by Cycle\", type=\"n\", col=\"black\", xlab=\"Cycle\", ylab=\"% Bases Soft Clipped\", ylim=c(0, 100))\n";
			var line6 = "for(i in 1:(length(yvals)-1))\n{\n";
			var line7 = "polygon(c(xvals[i] - 0.5, xvals[i] - 0.5, xvals[i] + 0.5, xvals[i] + 0.5), c(0, yvals[i], yvals[i], 0), col=cols[i], border=NA)\n";
			var line8 = "}\n";
			var line9 = "dev.off()\n";
...
exports.useR=useR;
exports.getReadBreakdown=getReadBreakdown;
