...
curl -k https://pinery.hpc.oicr.on.ca:8443/pinery/sequencerruns -o ./pinery-output/runs.out
curl -k https://pinery.hpc.oicr.on.ca:8443/pinery/samples -o ./pinery-output/samples.out

## Donors
qsub -cwd -b y -N DonorInfo -e error.log -o output.log -l h_vmem=8g "export NODE_PATH=$NODE_PATH; $node --max_old_space_size=8192 ./scripts/pinery-data-donor.js ./pinery-output/runs.out ./pinery-output/samples.out"
...
qsub -cwd -b y -N LibraryInfo -e error.log -o output.log -l h_vmem=8g "export NODE_PATH=$NODE_PATH; $node --max_old_space_size=8192 ./scripts/pinery-data-library.js ./pinery-output/runs.out ./pinery-output/samples.out skip.json receive_dates.json"

# Workflows
qsub -cwd -b y -N WorkflowInfo -hold_jid LibraryInfo -e error.log -o output.log -l h_vmem=8g "export NODE_PATH=$NODE_PATH; $node --max_old_space_size=8192 ./scripts/seqware-db-wfs.js $analysis_yaml"

## Projects
curl -k https://pinery.hpc.oicr.on.ca:8443/pinery/sample/projects -o ./pinery/projects.out

qsub -cwd -b y -N ProjInfo -e error.log -o output.log -l h_vmem=8g "export NODE_PATH=$NODE_PATH; $node --max_old_space_size=8192 ./scripts/pinery-data-project.js ./pinery-output/projects.out"