...
do
	run="$line"
	qsub -cwd -b y -N Perl_${run}_Report -e error.l -o output.log -l h_vmem=8g "export PERL5LIB=/oicr/local/analysis/lib/perl/pipe/lib/perl5; export PATH=${PATH}:/oicr/local/analysis/lib/perl/pipe/bin; perl RunReport.pl ${run} > /u/mcheng/browsertest/RunLaneReports/${run}_Report.json"
