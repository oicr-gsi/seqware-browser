...
do
	...
	qsub -cwd -b y -N Mongo_${run}_Report -e error.log -o output.log -l h_vmem=8g -hold_jid Perl_${run}_Report "export NODE_PATH=$NODE_PATH; $node --max_old_space_size=8192 ./scripts/run-report.js ./RunLaneReports/${run}_Report.json"
done < "$filename"
